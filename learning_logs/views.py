...
def edit_entry(request, entry_id):
    """To edit the content of an entry in the entries list"""
    ...
    if request.method != 'POST':
        # initial request displaying the content of the entry
        form = EntryForm(instance= entry)

    else:
        # request to edit the content of the entry
        form = EntryForm(instance= entry, data= request.POST)
        if form.is_valid():
            form.save()
            return redirect('learning_logs:topic', topic_id= topic.id)
    # display entry content to be edited
